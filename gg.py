...
class UIManager:
    ...
    def show_login_page(self):
        ...
        password_entry = self.create_label_entry(login_frame, "Shifre:")
        password_entry.config(show="*")

        image = tk.PhotoImage(file="C:\\Users\PC\Downloads\\finalsekil.png")
        img_label = tk.Label(login_frame, image=image, bg=BG_COLOR)
        ...
        def attempt_login():
            ...
            if username == USERNAME and password == PASSWORD:
                ...
                messagebox.showerror("Xeta", "Istifadeci adi ve ya shifre sefdir!")

        login_button = tk.Button(login_frame, text="Daxil Ol", command=attempt_login, font=("Arial", 12), bg=BUTTON_BG, fg=BUTTON_FG)
        login_button.pack(pady=10)

    def show_main_page(self):
        button_font = ("Arial", 12)
        add_button = tk.Button(self.root, text="Yeni Isci Elave Et", command=self.add_worker_window, font=button_font, bg=BUTTON_BG, fg=BUTTON_FG)
        add_button.pack(pady=10)

        list_button = tk.Button(self.root, text="Butun Ishcileri Goster", command=self.list_workers_window, font=button_font, bg=BUTTON_BG, fg=BUTTON_FG)
    ...
    def add_worker_window(self):
        ...
        add_window.title("Yeni Ishci Elave Et")
        ...
        position_combobox = self.create_combobox(add_window, "Vezife:", ["Muellim", "Muhendis", "Menecer", "Hekim", "Mudur", "Kassir", "Polis", "Ofissiant", "Ekspert", "Proqramist"])
        experience_combobox = self.create_combobox(add_window, "Is Tecrubesi:", ["1 Ilden Az", "1-3 Il", "3 Ilden Cox"])
        city_combobox = self.create_combobox(add_window, "Sheherler:", ["Baki", "Gence", "Sumqayit", "Mingachevir", "Lacin", "Semkir", "Qazax", "Kurdemir", "Daskesen", "Shusha", "Kelbecer", "Zengilan"])
        ...
        def save_worker():
            ...
            messagebox.showinfo("Ugur", "Yeni ishci elave olundu!")
            add_window.destroy()

        save_button = tk.Button(add_window, text="Saxla", command=save_worker, font=("Arial", 12), bg=BUTTON_BG, fg=BUTTON_FG)
    ...
    def list_workers_window(self):
        ...
        list_window.title("Butun Ishchiler")
        list_window.geometry("500x400")
        list_window.configure(bg=BG_COLOR)

        worker_listbox = tk.Listbox(list_window, font=("Arial", 12), bg=ENTRY_BG, fg=ENTRY_FG, width=50, height=15, selectbackground="#606060")
    ...
    def show_worker_details_window(self, worker):
        ...
        info = (
            ...
            f"Sheher: {worker['city']}\n"
            f"Unvan: {worker['address']}\n"
        )
        tk.Label(details_window, text="Ishchi Haqqinda Melumat", font=("Arial", 14, "bold"), bg=BG_COLOR, fg=FG_COLOR).pack(pady=10)
        tk.Label(details_window, text=info, font=("Arial", 12), bg=BG_COLOR, fg=FG_COLOR, justify="left").pack(pady=10)

        edit_button = tk.Button(details_window, text="Edit Et", command=lambda: self.edit_worker_window(worker), font=("Arial", 12), bg=BUTTON_BG, fg=BUTTON_FG)
        edit_button.pack(pady=10)

        delete_button = tk.Button(details_window, text="Sil", command=lambda: self.delete_worker(worker, details_window), font=("Arial", 12), bg=DELETE_BUTTON_BG, fg=BUTTON_FG)
        delete_button.pack(pady=10)

    def edit_worker_window(self, worker):
        edit_window = tk.Toplevel(self.root)
        edit_window.title("Edit Et")
        edit_window.geometry("450x400")
        edit_window.configure(bg=BG_COLOR)

        position_combobox = self.create_combobox(edit_window, "Vezife:", ["Muellim", "Muhendis", "Menecer", "Hekim", "Mudur", "Kassir", "Polis", "Ofissiant", "Ekspert", "Proqramist"])
        position_combobox.set(worker["position"])

        experience_combobox = self.create_combobox(edit_window, "Ish Tecrubesi:", ["1 Ilden Az", "1-3 Il", "3 Ilden Cox"])
        experience_combobox.set(worker["workExperience"])

        city_combobox = self.create_combobox(edit_window, "Sheherler:", ["Baki", "Gence", "Sumqayit", "Mingachevir", "Lacin", "Semkir", "Qazax", "Kurdemir", "Daskesen", "Shusha", "Kelbecer", "Zengilan"])
        ...
        def save_edits():
            ...
            if updated_data != worker:
                ...
                messagebox.showinfo("Ugur", "Ishchi melumatlari yenilendi!")
                edit_window.destroy()
            else:
                messagebox.showinfo("Məlumat", "Heç bir dəyişiklik edilmədi!")

        save_button = tk.Button(edit_window, text="Deyishiklikleri Saxla", command=save_edits, font=("Arial", 12), bg=BUTTON_BG, fg=BUTTON_FG)
        save_button.pack(pady=20)

    def delete_worker(self, worker, window):
        self.manager.delete_worker(worker)
        messagebox.showinfo("Ugur", "Ishci silindi!")
        window.destroy()
